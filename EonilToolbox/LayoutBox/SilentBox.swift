...
extension SilentBox {
	...
	public func shrinkenTo(newSize: (x: Scalar, y: Scalar)) -> SilentBox {
		let a = Swift.min(self.size.x, newSize.x)
		let b = Swift.min(self.size.y, newSize.y)
		return resizedTo((a,b))
	}
	public func shrinkenXTo(newSizeX: Scalar) -> SilentBox {
		return shrinkenTo((newSizeX, size.y))
	}
	public func shrinkenYTo(newSizeY: Scalar) -> SilentBox {
	...
	public func grownTo(newSize: (x: Scalar, y: Scalar)) -> SilentBox {
		let a = Swift.max(self.size.x, newSize.x)
		let b = Swift.max(self.size.y, newSize.y)
		return resizedTo((a,b))
	}
	public func grownXTo(newSizeX: Scalar) -> SilentBox {
		return grownTo((newSizeX, size.y))
	}
	public func grownYTo(newSizeY: Scalar) -> SilentBox {
		return grownTo((size.x, newSizeY))
	}
}
// MARK: - Overridins
extension SilentBox {
	public func splitAtX(x: Scalar) -> (min: SilentBox, max: SilentBox) {
